...
| `07i / 07k / 07m / 07o_*.py` | — | stylia NPG 2×3 slides (4 per organism): predictor comparison (`07i`) · **target prioritization** (`07k`, 6-panel synthesis: cumulative score, prioritization map, essential-vs-ligandable, neglected-vs-essential, target scorecard, funnel) · publications/CRISPRi (`07m`) · conditions (`07o`). `07j` (composite summary) is retired — folded into `07k`. |
