...
TSV_PATH = REPO_ROOT / "data" / "raw" / "proteome" / "UP000007841_HS11286.tsv"
OUT_PATH = REPO_ROOT / "data" / "processed" / "embeddings" / "kp_esmc600m_embeddings.npz"
...
def load_client(device: str):
    ...
    try:
        ...
        msg = str(exc).lower()
        if any(k in msg for k in ("401", "403", "gated", "token", "authenticate", "login")):
            sys.exit(
...
def main() -> None:
    ...
    print(
        ...
        f"to {args.out.relative_to(REPO_ROOT)}"
    )
    if skipped:
