"""2x3 overview of the ChEMBL bioactivity annotation (06a), direct vs orthology-transferred.

Same house style as the task-agnostic slides (05b/03b): stylia "slide" format, NPG palette, white
in-bar labels. Every panel is specific to the single `--organism` of the slide — there is no
content shared between the kp and ec slides. Distinguishes evidence measured on (essentially) the
protein itself — DIRECT, a ChEMBL target at >=95% identity — from evidence TRANSFERRED from a
lower-identity bacterial ortholog.

  1  potent targets by route             # proteins with <=1 uM / <=100 nM activity, direct vs transferred
  2  evidence transfer distance          best ChEMBL-target %identity distribution (95% = direct)
  ...
  6  evidence-tier coverage              proteins reachable via direct / bacterial / human ortholog

Reads output/results/<org>/<prefix>_chembl.csv (06a). Output: output/plots/06i_chembl_<prefix>.png
...
def load(prefix: str, org: str) -> pd.DataFrame:
    d = pd.read_csv(L.results_dir(org) / f"{prefix}_chembl.csv")
    d["has_direct"] = d["chembl_direct_has"].fillna(False).astype(bool)
    d["bact_n_potent"] = d["chembl_bact_n_potent"].fillna(0)
    d["bact_n_comp"] = d["chembl_bact_n_compounds"].fillna(0)
    d["dir_n_potent"] = d["chembl_direct_n_potent"].fillna(0)
    d["human_n_comp"] = d["chembl_human_n_compounds"].fillna(0)
...
def main() -> None:
    ...
    ax = axs.next()
    dir_pch = d["chembl_direct_best_pchembl"].fillna(0)
    bact_pch = d["chembl_bact_best_pchembl"].fillna(0)
    cats = ["≤1 µM\n(pChEMBL≥6)", "≤100 nM\n(pChEMBL≥7)"]
    xc = range(len(cats))
    direct = [int((d["dir_n_potent"] > 0).sum()),
              int((d["has_direct"] & (dir_pch >= 7)).sum())]
    ortho = [int(((d["bact_n_potent"] > 0) & ~d["has_direct"]).sum()),
             int((~d["has_direct"] & (bact_pch >= 7)).sum())]
    ...
    # ---- panel 2: best ChEMBL-target sequence identity (this organism) ----
    ax = axs.next()
    pid = d.loc[d["has_bact"], "chembl_bact_best_pident"].dropna()
    ...
    ax.set_xlabel("Best ChEMBL-target % identity"); ax.set_ylabel("Number of proteins")
    ax.set_title(f"Evidence transfer distance — {orgname}")

    # ---- BL: top targets by # potent compounds, coloured by route (one organism) ----
    ax = axs.next()
    ...
    # log x — # potent compounds spans orders of magnitude (e.g. heavily-screened β-lactamases)
    ax.set_xscale("log")
    ax.set_xlim(1, vals.max() * 2.5)
    ...
    # ---- BR: evidence source organisms, one vivid colour per organism ----
    ax = axs.next()
    sub = d[d["has_bact"]].copy()
    sub["src"] = sub["chembl_bact_best_organism"].map(short_org)
    ...
    # ---- panel 6: how many proteins have ChEMBL data, and via which homolog (this organism) ----
    ax = axs.next()
    tiers = ["Direct\n(≥95% id)", "Bacterial\nhomolog", "Human\nhomolog"]
    ...
    ax.set_ylabel("Proteins with ChEMBL data"); ax.set_xlabel("")
    ...
    ax.text(0.5, -0.24,
            ...
            f"The other {n_none:,} of {len(d):,} proteins have no ChEMBL data.",
            transform=ax.transAxes, ha="center", va="top", color="#777777", fontsize="small")

    out = REPO_ROOT / "output" / "plots" / f"06i_chembl_{prefix}.png"
